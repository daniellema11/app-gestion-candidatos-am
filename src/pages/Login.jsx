...
const Login = () => {
  ...
  const findUser = () => {
    let user = getUsers.find(
      (item) => getEmail === item.email && getPassword === item.password,
    );
    return user;
  };
  ...
  function signIn() {
    ...
    if (findUser()) {
      ...
      redirect(
        findUser().fullName + " Bienvenido al sistema...",
        "/dashboard",
        "success",
      );
    } else {
        ...
        <div class="w-full max-w-md bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 p-8 lg:p-10">
          <div class="flex flex-col items-center mb-8">
            ...
            <p class="text-slate-500 dark:text-slate-400 text-sm mt-2 text-center">
              Manage your applications and profile in one place.
            </p>
          </div>
          <form class="space-y-5">
            <div class="flex flex-col gap-2">
              ...
              <div class="relative">
                <input
                  ...
                  class="w-full px-4 py-3 rounded border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all outline-none placeholder:text-slate-400 dark:placeholder:text-slate-500"
                  id="email"
                  name="email"
            ...
            <div class="flex flex-col gap-2">
              ...
              <div class="relative flex items-center">
                ...
                />
                <button
                  class="absolute right-3 text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300"
                  type="button"
                >
            ...
            <div class="flex items-center gap-2">
              <input
                class="w-4 h-4 text-primary border-slate-300 dark:border-slate-700 rounded focus:ring-primary/20"
                id="remember"
                type="checkbox"
              />
              <label
            ...
            <button
              ...
              class="w-full bg-primary hover:bg-primary/90 text-dark font-semibold py-3 px-4 border-black rounded flex items-center justify-center gap-2"
              type="button"
            >
              Sign In
